...
<head>
    ...
    <title>Dokter - Sistem Informasi Klinik</title>
    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
        ...
        <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
            ...
            <div class="flex items-center justify-between p-6 border-b border-gray-800">
                <span class="text-2xl font-bold tracking-wider text-pink-500">Dokter Klinik</span>
            ...
            <nav class="flex-1 mt-6 px-4 space-y-2 overflow-y-auto">
                ...
                {{ request()->routeIs('dokter.dashboard') ? 'bg-pink-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                ...
                <div class="pt-4 pb-1">
                    <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Pelayanan Pasien</p>
                </div>

                {{-- ANTRIAN --}}
                <a href="{{ route('dokter.queues.index') }}"
                    class="flex items-center px-4 py-3 rounded-lg transition duration-200
                {{ request()->routeIs('dokter.queues*') ? 'bg-pink-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z">
                        </path>
                    </svg>
                    <span class="font-medium">Antrian Pemeriksaan</span>
                </a>

                {{-- PASIEN SAYA --}}
                <a href="{{ route('dokter.patients.index') }}"
                    class="flex items-center px-4 py-3 rounded-lg transition duration-200 {{ request()->routeIs('dokter.patients*') ? 'bg-pink-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="pt-4 pb-1">
                    <p class="px-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Administrasi</p>
                </div>

                {{-- TRANSAKSI --}}
                <a href="{{ route('dokter.transactions.index') }}"
                    class="flex items-center px-4 py-3 rounded-lg transition duration-200
                {{ request()->routeIs('dokter.transactions*') ? 'bg-pink-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('dokter.reports.index') }}"
                    class="flex items-center px-4 py-3 rounded-lg transition duration-200 {{ request()->routeIs('dokter.reports*') ? 'bg-pink-600 text-white shadow-md' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="p-4 border-t border-gray-800">
                <div class="flex items-center">
                    <div class="ml-3">
                        <p class="text-sm font-medium text-white">{{ Auth::user()->name ?? 'Dokter' }}</p>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header class="flex justify-between items-center py-4 px-6 bg-white border-b lg:hidden">
                <div class="flex items-center">
                    ...
                    <span class="ml-4 text-lg font-semibold text-gray-900">Dashboard Dokter</span>
                </div>
            </header>
