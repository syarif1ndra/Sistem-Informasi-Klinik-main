    ...
    <div x-data="{
        ...
        <div class="bg-white rounded-xl shadow-md p-4 md:p-6 mb-6 border border-gray-100">
            <form method="GET" action="{{ route('admin.users.index') }}"
                class="flex flex-col lg:flex-row gap-4 items-stretch lg:items-end">

                <div class="flex-grow">
                    <label for="search" class="block text-sm font-semibold text-gray-700 mb-1.5">Cari Nama atau
                        Email</label>
                    <div class="relative">
                        ...
                        <input type="text" name="search" id="search" value="{{ request('search') }}"
                            class="pl-10 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 block w-full sm:text-sm border-gray-300 rounded-lg py-2.5 border shadow-sm transition-all"
                            placeholder="Contoh: Budi atau budi@example.com">
                    </div>
                </div>

                <div class="w-full lg:w-64">
                    <label for="role_filter" class="block text-sm font-semibold text-gray-700 mb-1.5">Filter Role</label>
                    <select name="role" id="role_filter"
                        class="block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 sm:text-sm rounded-lg border shadow-sm transition-all">
                ...
                <div class="flex items-center gap-2">
                    <button type="submit"
                        class="flex-1 lg:flex-none inline-flex justify-center py-2.5 px-6 border border-transparent shadow-sm text-sm font-bold rounded-lg text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition-all active:scale-95">
                        Terapkan
                    </button>

                    @if (request('search') || request('role'))
                        <a href="{{ route('admin.users.index') }}"
                            class="flex-1 lg:flex-none inline-flex justify-center py-2.5 px-6 border border-gray-300 shadow-sm text-sm font-bold rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 transition-all active:scale-95">
                            Reset
                        </a>
                    @endif
                </div>
                        ...
                        @forelse($users as $user)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    ...
                                    @endphp
                                    <span
                                        class="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full border {{ $roleColor }} shadow-sm">
                                        {{ ucfirst($user->role) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        @if ($user->id !== auth()->id())
                                            <button
                                                @click="openDeleteModal($el.closest('tr').querySelector('form'), '{{ $user->name }}')"
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-400 rounded-md cursor-not-allowed text-xs font-semibold"
        ...
        <div x-show="showCreateModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div @click="showCreateModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-75 transition-opacity">
                </div>

                <div
                    class="bg-white rounded-xl shadow-2xl transform transition-all sm:max-w-lg sm:w-full z-10 overflow-hidden">
                    <div
                        class="bg-gradient-to-r from-pink-600 to-rose-600 px-6 py-4 flex justify-between items-center text-white">
                    ...
                    <form action="{{ route('admin.users.store') }}" method="POST" class="p-6">
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            <button type="button" @click="showCreateModal = false"
                                class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-semibold">Batal</button>
                            <button type="submit"
                                class="px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 shadow-md transition font-bold">Simpan
                                User</button>
                        </div>
                    </form>
                </div>
        ...
        <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div @click="showEditModal = false" class="fixed inset-0 bg-gray-600 bg-opacity-75 transition-opacity">
                </div>
                    ...
                    <form :action="editRoute" method="POST" class="p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="pt-2 border-t border-gray-100">
                                <p class="text-xs text-gray-500 mb-2 italic">* Kosongkan password jika tidak ingin mengubah
                                </p>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            <button type="button" @click="showEditModal = false"
                                class="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-semibold">Batal</button>
                            <button type="submit"
                                class="px-6 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 shadow-md transition font-bold">Update
                                User</button>
                        </div>
                    </form>
                </div>
