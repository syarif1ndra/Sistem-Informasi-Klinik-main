...
@section('title', 'Lengkapi Data Pasien')
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl shadow-gray-200/50 overflow-hidden border border-gray-100">
                <div class="bg-gradient-to-br from-pink-500 via-rose-500 to-rose-600 px-8 py-10 relative">
                    ...
                    <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h2 class="text-3xl font-extrabold text-white tracking-tight">Lengkapi Data Pasien</h2>
                            <p class="text-pink-100 mt-1 font-medium italic opacity-90">Satu langkah lagi untuk mulai
                ...
                <form action="{{ route('user.patient.store') }}" method="POST" class="p-8 md:p-10 space-y-8">
                    @csrf
                    ...
                    @if ($errors->any())
                        <div
                            ...
                            <p class="text-sm text-rose-700 font-semibold">Mohon lengkapi formulir dengan benar.</p>
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div class="col-span-2 space-y-2">
                            ...
                            @error('name') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="space-y-2">
                            <label for="nik" class="block text-sm font-bold text-gray-700 ml-1">NIK (16 Digit)</label>
                            <input type="text" name="nik" id="nik" value="{{ old('nik') }}"
                                ...
                                placeholder="3210..." minlength="16" maxlength="16" pattern="\d{16}"
                                title="NIK harus terdiri dari 16 angka" required>
                            @error('nik') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="dob" id="dob" value="{{ old('dob') }}"
                                class="w-full px-4 py-3.5 bg-white rounded-xl border-2 border-gray-100 text-gray-700 font-semibold focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition-all outline-none"
                                required>
                            @error('dob') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            @error('gender') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="space-y-2">
                            <label for="phone" class="block text-sm font-bold text-gray-700 ml-1">Nomor HP</label>
                            <input type="text" name="phone" id="phone" value="{{ old('phone') }}"
                                class="w-full px-4 py-3.5 bg-white rounded-xl border-2 border-gray-100 text-gray-700 font-semibold focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition-all outline-none"
                                placeholder="08..." required>
                            @error('phone') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            @error('address') <p class="text-xs text-rose-500 font-medium ml-1 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <a href="{{ route('dashboard') }}"
                            class="flex items-center justify-center px-6 py-3.5 rounded-xl text-gray-400 font-bold hover:bg-gray-100 hover:text-gray-600 transition-all active:scale-95 group">
                        ...
                        <button type="submit"
                            class="w-full md:w-auto px-12 py-4 bg-gradient-to-r from-pink-500 to-rose-600 text-white rounded-2xl font-black shadow-xl shadow-rose-200 hover:shadow-2xl hover:shadow-rose-300 transform hover:-translate-y-1 transition-all active:scale-95">
                            Simpan & Lanjutkan
                        </button>
                    </div>
                </form>
