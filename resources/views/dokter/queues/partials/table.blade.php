@forelse($queues as $queue)
    <tr id="row-{{ $queue->id }}"
        class="{{ $queue->status == 'calling' ? 'bg-yellow-50/50' : 'hover:bg-pink-50/30 transition-colors group' }}">
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="text-sm font-bold text-gray-900">{{ $queue->patient_name }}</div>
        </td>
...
@empty
    <tr>
        <td colspan="5" class="px-6 py-12 text-center text-gray-500 font-medium">Tidak ada data antrian untuk hari ini.
        </td>
    </tr>
@endforelse