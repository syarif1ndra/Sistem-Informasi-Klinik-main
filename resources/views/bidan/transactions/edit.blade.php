...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="billingSystem()">
        ...
        <form action="{{ route('bidan.transactions.update', $transaction->id) }}" method="POST" id="billingForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 sticky top-4">
                        ...
                        <div class="border-t-2 border-dashed border-gray-200 pt-4 bg-gray-50 -mx-6 -mb-6 p-6 rounded-b-xl">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="notes" rows="3" placeholder="Contoh: Pasien alergi penisilin..."
                                    class="w-full rounded-lg border-gray-300 focus:border-pink-500 focus:ring-pink-500 shadow-sm text-sm p-2">{{ old('notes', $transaction->notes) }}</textarea>
                            </div>
