...
@section('title', 'Edit Skrining - ' . $patient->name)
...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Edit Skrining &amp; Diagnosis</h1>
        ...
        <form action="{{ route('bidan.patients.screenings.update', [$patient, $screening]) }}" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="examined_at"
                        value="{{ old('examined_at', \Carbon\Carbon::parse($screening->examined_at)->format('Y-m-d\TH:i')) }}"
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="height" value="{{ old('height', $screening->height) }}"
                            placeholder="cth: 160"
                    ...
                    <div>
                        ...
                        <input type="text" name="weight" value="{{ old('weight', $screening->weight) }}"
                            placeholder="cth: 55"
                            class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">
                    </div>
                    <div>
                        ...
                        <input type="text" name="blood_pressure"
                            value="{{ old('blood_pressure', $screening->blood_pressure) }}" placeholder="cth: 120/80"
                            class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Suhu Tubuh (°C)</label>
                        <input type="text" name="temperature" value="{{ old('temperature', $screening->temperature) }}"
                            placeholder="cth: 36.5"
                            class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Denyut Nadi (bpm)</label>
                        <input type="text" name="pulse" value="{{ old('pulse', $screening->pulse) }}" placeholder="cth: 80"
                            class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">
                    </div>
                    <div>
                        ...
                        <input type="text" name="respiratory_rate"
                            value="{{ old('respiratory_rate', $screening->respiratory_rate) }}" placeholder="cth: 20"
                            class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="main_complaint" rows="2"
                        class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">{{ old('main_complaint', $screening->main_complaint) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Riwayat Penyakit</label>
                    <textarea name="medical_history" rows="2"
                        class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">{{ old('medical_history', $screening->medical_history) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Diagnosis Utama</label>
                    <textarea name="diagnosis_text" rows="2"
                        class="w-full px-4 py-3 rounded-xl border-2 border-gray-100 text-gray-700 focus:ring-4 focus:ring-pink-500/10 focus:border-pink-500 transition outline-none">{{ old('diagnosis_text', $screening->diagnosis_text) }}</textarea>
                </div>
                <div>
                    ...
                    <select name="icd10_codes[]" id="icd10Select" multiple style="width:100%">
                        @foreach($icd10Codes as $code)
                            <option value="{{ $code->id }}" {{ in_array($code->id, old('icd10_codes', $selectedIds)) ? 'selected' : '' }}>
                                {{ $code->code }} — {{ $code->name }}
                            </option>
                        @endforeach
                    </select>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#icd10Select').select2({ placeholder: 'Cari kode ICD-10...', allowClear: true, width: '100%' });
        });
    </script>
@endpush