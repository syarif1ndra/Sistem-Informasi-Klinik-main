    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div x-data="{ open: false }" class="relative z-10" @click.away="open = false">
            ...
            <div x-show="open"
                ...
                style="display: none;">
                <div class="py-1">
                    <a href="{{ route('bidan.reports.exportExcel', ['start_date' => $startDate, 'end_date' => $endDate, 'payment_method' => $paymentMethod ?? 'all', 'search' => $search ?? '']) }}"
                        target="_blank"
                    ...
                    <a href="{{ route('bidan.reports.exportPdf', ['start_date' => $startDate, 'end_date' => $endDate, 'payment_method' => $paymentMethod ?? 'all', 'search' => $search ?? '']) }}"
                        target="_blank"
                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-2">
